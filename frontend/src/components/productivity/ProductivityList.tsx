...
import { fetchProductivityRecords, deleteProductivityRecord } from '../../redux/productivity/operations';
...
const ProductivityList: React.FC = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-md rounded-lg mt-6">
      <h2 className="text-2xl font-semibold text-gray-800 mb-4">Productivity Records</h2>
      <ul className="space-y-4">
        {records.map((record) => (
          <li key={record._id} className="p-4 border border-gray-200 rounded-lg bg-gray-50">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-lg font-semibold text-gray-700">
                Department: {typeof record.departmentId === 'object' ? (record.departmentId as { name: string }).name : record.departmentId}
              </h3>
            ...
            <p className="text-gray-600">
              <span className="font-semibold">Date:</span> {new Date(record.date).toLocaleDateString()}
            </p>
            <p className="text-gray-600">
              <span className="font-semibold">Units Completed:</span> {record.unitsCompleted}
            </p>
            <p className="text-gray-600">
              <span className="font-semibold">Earnings:</span> ${record.totalEarnings.toFixed(2)}
            </p>
          </li>
        ))}
      </ul>
      ...
      {editingRecord && (
        <div className="mt-6">
          <ProductivityUpdateForm record={editingRecord} onClose={closeUpdateForm} />
        </div>
      )}
    </div>
  );
