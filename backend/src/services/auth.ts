...
import { FIFTEEN_MINUTES, THIRTY_DAYS, TEMPLATES_DIR } from '../constants/constants';
...
export const loginUser = async (payload: UserPayload) => {
  ...
  const { accessToken, refreshToken, accessTokenValidUntil, refreshTokenValidUntil } = createSession();
...
export const refreshUsersSession = async ({ sessionId, refreshToken }: { sessionId: string; refreshToken: string }) => {
